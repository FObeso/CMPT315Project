...
const ChooseCar = () => {
  ...
  const branchID = Number.parseInt(localStorage.getItem("branchID"));
  ...
  async function generate_avail_car_id(rented_car_list) {
    ...
    for (var i = 0; i < availiable_cars.length; i++) {
      for (var j = 0; j < carList.length; j++) {
        console.log(carList[j].id, availiable_cars[i]);
        console.log(branchID);
        if (
          availiable_cars[i] === carList[j].id &&
          carList[j].BranchID === parseInt(branchID)
        ) {
          availCars.push(carList[j]);
        }
      }
    }
    console.log(availCars);
    setCars(availCars);
  }
