...
const Rentals = () => {
  ...
  async function generate_avail_car_id(rented_car_list) {
    ...
    var availiable_cars = car_list.filter(
      (x) => rented_car_list.indexOf(x) === -1
    );
  ...
  async function find_cars_availiable(startDate, endDate) {
    ...
    for (let i = 0; i < length; i++) {
      ...
      var date_1 = is_date_range_valid(
        data[i].dateFrom,
        data[i].dateTo,
        startDate
      );
      var date_2 = is_date_range_valid(
        data[i].dateFrom,
        data[i].dateTo,
        endDate
      );
      ...
      if (
        date_1 == false &&
        date_2 == false &&
        data[i].rentalEmployeeID != null &&
        data[i].rentalBranchID != null
      ) {
    ...
    <div>
      <Box sx={style}>
        <form onSubmit={handleSubmit}>
          <div style={{ width: "700" }}>
            <label htmlFor="">Choose Pickup Branch</label>
            <select
              style={{
              ...
              className="w-80 m-2 mt-5 p-2 border-2 rounded-md border-primary "
            >
              <option value=""></option>
              {branches.map((branch) => (
      ...
      <div>
        dateFrom: {startDate.toLocaleDateString()} dateTo:{" "}
